...
export function loadTestEnv() {
  ...
  process.env.CRYPTOMUS_API_KEY = process.env.CRYPTOMUS_API_KEY || 'mock-cryptomus-api-key';
  process.env.CRYPTOMUS_MERCHANT_ID = process.env.CRYPTOMUS_MERCHANT_ID || 'mock-merchant-id';
  ...
  // Ethereum provider
  process.env.ETH_PROVIDER_URL = process.env.ETH_PROVIDER_URL || 'https://cloudflare-eth.com';
}
...
export function skipIfNoCredentials(serviceName: string, envVars: string[]) {
  const missing = envVars.filter(v => !process.env[v] || process.env[v]?.startsWith('mock-'));
 ...
 */
export function isServiceAvailable(service: 'cryptomus' | 'email' | 'stripe' | 'blockchain'): boolean {
  ...
  return requiredVars.every(v => process.env[v] && !process.env[v]?.startsWith('mock-'));
}
