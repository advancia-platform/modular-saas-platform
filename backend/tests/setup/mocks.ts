 ...
 */
export const mockBlockchainNetwork = {
...
export const feeCalculator = {
  calculateNetworkFee: (currency: string, gasPrice?: string) => {
    const fees = {
      BTC: '0.0001',
      ETH: gasPrice ? (parseFloat(gasPrice) * 21000 / 1e18).toString() : '0.001',
      USDT: '0.5',
    };
  ...
  calculateTotalFee: (amount: string, currency: string, gasPrice?: string) => {
    const networkFee = parseFloat(feeCalculator.calculateNetworkFee(currency, gasPrice));
    const serviceFee = parseFloat(feeCalculator.calculateServiceFee(amount, currency));
...
export const addressValidators = {
  ...
  isValidCryptoAddress: (address: string, currency: string): boolean => {
    if (currency === 'BTC') return addressValidators.isValidBitcoinAddress(address);
    if (currency === 'ETH' || currency === 'USDT') return addressValidators.isValidEthereumAddress(address);
    return false;
  },
};
