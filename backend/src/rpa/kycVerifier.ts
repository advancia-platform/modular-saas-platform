  ...
  async verifyDocument(
    ...
    try {
      ...
      if (
        ...
        console.log(
          `✅ KYC approved for user ${userId} (confidence: ${result.confidence.toFixed(2)})`,
        );
      } else {
        ...
        console.log(
          `⚠️  KYC requires manual review for user ${userId} (confidence: ${result.confidence.toFixed(2)})`,
        );
      }
    } catch (error) {
      result.errors.push(
