...
export const transactionSchema = z.object({
  ...
  type: z.enum(['credit', 'debit'], { errorMap: () => ({ message: 'Invalid transaction type' }) }),
});
...
export const adminUserCreateSchema = z.object({
  ...
  firstName: z.string().min(1, 'First name required').max(100).trim(),
  lastName: z.string().min(1, 'Last name required').max(100).trim(),
  role: z.enum(['admin', 'super_admin'], { errorMap: () => ({ message: 'Invalid role' }) }),
});
...
export const settingsSchema = z.object({
  ...
  type: z.enum(['string', 'number', 'boolean', 'json'], { errorMap: () => ({ message: 'Invalid setting type' }) }),
});
...
export const paginationSchema = z.object({
  ...
  limit: z.number().int().min(1, 'Limit must be at least 1').max(100, 'Limit too large').default(10),
