...
router.get("/tier", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const nextTierRequirement = nextTier
      ? USER_TIERS[nextTier as keyof typeof USER_TIERS].minTokens
      : null;
    ...
    res.json({
      ...
      nextTier,
      tokensUntilNextTier: nextTierRequirement
        ? nextTierRequirement - totalTokens
        : null,
    });
  } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        totalRewards: unlockedAchievements.reduce(
          (sum, a) => sum + a.reward,
          0,
        ),
      });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const cryptoPurchases = await prisma.crypto_wallets.count({
        where: { userId },
      });
