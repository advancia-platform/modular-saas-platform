...
function parsePriceFromMetadata(meta?: string | null): number | null {
  try {
    ...
    if (j && typeof j.priceUSD === "number" && j.priceUSD > 0)
      return j.priceUSD;
  } catch {}
  return null;
}
...
async function getDebitCardPrice(): Promise<number> {
  ...
  const row = await prisma.system_status.findFirst({
    where: { serviceName: "debit-card" },
  });
...
router.post(
  ...
  async (req, res) => {
    ...
    const existing = await prisma.system_status.findFirst({
      where: { serviceName: "debit-card" },
    });
    const payload = {
    ...
    if (existing) {
      ...
      await prisma.system_status.create({
        data: { ...payload, id: randomUUID(), updatedAt: new Date() },
      });
    }
...
router.post("/order", authenticateToken as any, async (req: any, res) => {
  ...
  if (!fullName || !address || !phone)
    return res
      .status(400)
      .json({ error: "fullName, address and phone are required" });
  ...
  try {
    ...
    try {
      ioRef?.to("admins").emit("admin:debit-card:order", {
        ticketId: ticket.id,
        userId,
        fullName,
      });
